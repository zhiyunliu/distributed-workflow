...
func NewInstanceStateService(repo api.WorkflowRepository) *InstanceStateServiceImpl {
	return &InstanceStateServiceImpl{repo: repo}
}

// SetScheduler 注入调度服务（解决 SchedulerService↔InstanceStateService 循环依赖）
func (s *InstanceStateServiceImpl) SetScheduler(sched api.SchedulerService) {
	s.sched = sched
}
