...
-- 表单定义表
IF OBJECT_ID('workflow_form_definition', 'U') IS NULL
BEGIN
    CREATE TABLE workflow_form_definition (
        ...
        CONSTRAINT PK_workflow_form_definition PRIMARY KEY (form_id)
    );
END
GO
...
BEGIN
    CREATE INDEX idx_form_def_status ON workflow_form_definition(status, deleted);
END
GO

-- 表单实例表
IF OBJECT_ID('workflow_form_instance', 'U') IS NULL
BEGIN
    CREATE TABLE workflow_form_instance (
        ...
        created_at           DATETIME2     NOT NULL DEFAULT GETDATE(),
        updated_at           DATETIME2     NOT NULL DEFAULT GETDATE(),
        CONSTRAINT PK_workflow_form_instance PRIMARY KEY (instance_id)
    );
END
GO
...
BEGIN
    CREATE INDEX idx_form_instance_workflow ON workflow_form_instance(workflow_instance_id, node_id);
END
GO

-- 表单版本历史表
IF OBJECT_ID('workflow_form_version_history', 'U') IS NULL
BEGIN
    CREATE TABLE workflow_form_version_history (
        ...
        created_by VARCHAR(64)   NOT NULL,
        created_at DATETIME2     NOT NULL DEFAULT GETDATE(),
        CONSTRAINT PK_workflow_form_version_history PRIMARY KEY (id),
        CONSTRAINT UQ_workflow_form_version UNIQUE (form_id, version)
    );
END
GO
...
-- 审批委托表
IF OBJECT_ID('workflow_approval_delegate', 'U') IS NULL
BEGIN
    CREATE TABLE workflow_approval_delegate (
        id           BIGINT     IDENTITY(1,1) NOT NULL,
        ...
        created_at   DATETIME2  NOT NULL DEFAULT GETDATE(),
        updated_at   DATETIME2  NOT NULL DEFAULT GETDATE(),
        CONSTRAINT PK_workflow_approval_delegate PRIMARY KEY (id)
    );
END
GO
...
BEGIN
    CREATE INDEX idx_approval_delegate_delegator ON workflow_approval_delegate(delegator_id, status);
END
GO

-- 审批抄送表
IF OBJECT_ID('workflow_approval_cc', 'U') IS NULL
BEGIN
    CREATE TABLE workflow_approval_cc (
        id                   BIGINT     IDENTITY(1,1) NOT NULL,
        ...
        CONSTRAINT PK_workflow_approval_cc PRIMARY KEY (id)
    );
END
GO
...
BEGIN
    CREATE INDEX idx_approval_cc_user ON workflow_approval_cc(cc_user_id, is_read);
END
GO

-- 审批加签记录表
IF OBJECT_ID('workflow_approval_add_sign', 'U') IS NULL
BEGIN
    CREATE TABLE workflow_approval_add_sign (
        id                   BIGINT        IDENTITY(1,1) NOT NULL,
        workflow_instance_id VARCHAR(64)   NOT NULL,                  -- 流程实例ID
        ...
        created_at           DATETIME2     NOT NULL DEFAULT GETDATE(),
        CONSTRAINT PK_workflow_approval_add_sign PRIMARY KEY (id)
    );
END
GO
...
BEGIN
    CREATE INDEX idx_add_sign_instance ON workflow_approval_add_sign(workflow_instance_id, node_id);
END
GO
...
-- 流程指标日汇总表
IF OBJECT_ID('workflow_stats_workflow_daily', 'U') IS NULL
BEGIN
    CREATE TABLE workflow_stats_workflow_daily (
        id                    BIGINT      IDENTITY(1,1) NOT NULL,
        ...
        CONSTRAINT PK_workflow_stats_workflow_daily PRIMARY KEY (id),
        CONSTRAINT UQ_workflow_stats_workflow_date UNIQUE (workflow_id, stat_date)
    );
END
GO
...
BEGIN
    CREATE INDEX idx_stats_workflow_date ON workflow_stats_workflow_daily(stat_date);
END
GO

-- 审批指标日汇总表
IF OBJECT_ID('workflow_stats_approval_daily', 'U') IS NULL
BEGIN
    CREATE TABLE workflow_stats_approval_daily (
        id                   BIGINT      IDENTITY(1,1) NOT NULL,
        ...
        CONSTRAINT PK_workflow_stats_approval_daily PRIMARY KEY (id),
        CONSTRAINT UQ_workflow_stats_approval_date UNIQUE (user_id, stat_date)
    );
END
GO
...
BEGIN
    CREATE INDEX idx_stats_approval_date ON workflow_stats_approval_daily(stat_date);
END
GO
...
-- 第三方平台OAuth绑定表
IF OBJECT_ID('workflow_oauth_bindings', 'U') IS NULL
BEGIN
    CREATE TABLE workflow_oauth_bindings (
        id               BIGINT         IDENTITY(1,1) NOT NULL,
        ...
        created_at       DATETIME2      NOT NULL DEFAULT GETDATE(),
        updated_at       DATETIME2      NOT NULL DEFAULT GETDATE(),
        CONSTRAINT PK_workflow_oauth_bindings PRIMARY KEY (id),
        CONSTRAINT UQ_workflow_oauth_platform_openid UNIQUE (platform, open_id)
    );
END
GO
...
BEGIN
    CREATE INDEX idx_oauth_sys_user ON workflow_oauth_bindings(sys_user_id, platform);
END
GO
...
-- 插件信息表（插件市场）
IF OBJECT_ID('workflow_plugin_info', 'U') IS NULL
BEGIN
    CREATE TABLE workflow_plugin_info (
        ...
        created_at     DATETIME2      NOT NULL DEFAULT GETDATE(),
        updated_at     DATETIME2      NOT NULL DEFAULT GETDATE(),
        CONSTRAINT PK_workflow_plugin_info PRIMARY KEY (plugin_id),
        CONSTRAINT UQ_workflow_plugin_name UNIQUE (plugin_name)
    );
END
GO

-- 插件安装配置表
IF OBJECT_ID('workflow_plugin_config', 'U') IS NULL
BEGIN
    CREATE TABLE workflow_plugin_config (
        id             BIGINT        IDENTITY(1,1) NOT NULL,
        ...
        updated_at     DATETIME2     NOT NULL DEFAULT GETDATE(),
        CONSTRAINT PK_workflow_plugin_config PRIMARY KEY (id),
        CONSTRAINT UQ_workflow_plugin_config_id UNIQUE (plugin_id)
    );
END
GO
