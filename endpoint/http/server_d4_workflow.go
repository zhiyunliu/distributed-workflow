...
func (s *Server) saveWorkflowDraft(c *gin.Context) {
	...
	if def.ID == "" {
		id, err := s.svc.CreateWorkflow(&def)
		if err != nil {
	...
	version, err := s.svc.UpdateWorkflow(&def)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"code": 500, "message": err.Error()})
		return
	}
...
func (s *Server) listWorkflowVersionsV2(c *gin.Context) {
	id := c.Param("id")
	versions, err := s.svc.ListWorkflowVersions(id)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"code": 500, "message": err.Error()})
		return
	}
...
func (s *Server) getWorkflowVersionByID(c *gin.Context) {
	...
	wv, err := s.svc.GetWorkflowVersion(workflowID, version)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"code": 500, "message": err.Error()})
		return
	}
...
func (s *Server) rollbackWorkflowDef(c *gin.Context) {
	...
	newVersion, err := s.svc.RollbackWorkflow(req.WorkflowID, req.TargetVersion, operator)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"code": 500, "message": err.Error()})
		return
	}
...
func (s *Server) getInstanceTopology(c *gin.Context) {
	id := c.Param("id")
	instance, err := s.svc.GetWorkflowInstance(id)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"code": 500, "message": err.Error()})
		return
	}
	def, err := s.svc.GetWorkflow(instance.WorkflowID)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"code": 500, "message": err.Error()})
		return
	}
...
func (s *Server) getNodeStatesInternal(instanceID string) (interface{}, error) {
	...
	// 实际可在 InstanceStateService 中扩展 GetInstanceNodeStates
	instance, err := s.svc.GetWorkflowInstance(instanceID)
	if err != nil {
		return nil, err
	}
	return instance, nil
...
func (s *Server) retryInstanceNode(c *gin.Context) {
	...
	username, _ := c.Get("username")
	operator, _ := username.(string)
	if err := s.svc.RetryNode(id, nodeID, operator); err != nil {
		c.JSON(http.StatusOK, gin.H{"code": 500, "message": err.Error()})
		return
	}
