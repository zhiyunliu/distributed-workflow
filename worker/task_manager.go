...
// TaskManager 实现 interfaces.TaskExecutionManager
// 按节点类型提供并发限制（semaphore），每个任务独立 goroutine 执行
type TaskManager struct {
	container interfaces.NodeExecutorContainer
...
func NewTaskManager(
	...
	caps map[string]int,
) interfaces.TaskExecutionManager {
	return &TaskManager{
		container: container,
		engineRPC: reporter,
...
func (m *TaskManager) runTask(ctx context.Context, cancel context.CancelFunc, key string, task *types.Task, executor interfaces.NodeExecutor) {
	defer cancel()
	defer func() {
		m.mu.Lock()
